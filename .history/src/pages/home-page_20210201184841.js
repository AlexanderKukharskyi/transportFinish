...
const handleSelect = (value, cb) => {
  cb(value);
  const cities = Cities.filter(city => city.id !== +value)
  setCityList(cities);
}
  ...
  return (
    <form onSubmit={handleSubmit} className="request-form">
      <div className="request-form__item">
        ...
        <select onChange={(event) => handleSelect(event.target.value, setFromCity)} value={fromCity} required> 
          <option disabled value="">Выбери</option>
          {cityList.map(city => <option key={city.id} value={city.id}>{city.title}</option>)}
        </select>
      </div>

      <div className="request-form__item">
        <span className="request-form__label">Куда:</span>
        <select onChange={(event) => handleSelect(event.target.value, setToCity)} value={toCity} required> 
        <option disabled value="">Выбери</option>
        {cityList.map(city => <option key={city.id} value={city.id}>{city.title}</option>)}
        </select>
      </div>
